...
function usage() {
  console.log("Usage: node scripts/google.mjs [-t6] [info]");
}
...
async function main() {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (Number(row.google_crawled ?? 0) > 0) {
        totalCrawled += 1;
      } else {
        totalSubmitted += 1;
        urls.push(url);
      }
      console.info(`${url} - ${row.google_crawled}`);
    }

    if (!info && urls.length > 0) {
      if (!baseUrl) {
        console.info("Skipping Google submission because PUBLIC_APP_URL is not set");
      } else {
