...
import { appendNotice, resolveRelativeReturnTo, resumeNow } from "@/services/resumes/form";
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (!session?.session?.id || !session.user?.id) {
    return new Response(null, {
      status: 302,
      headers: { location: "/auth/login?next=/app/resumes/list" },
    });
  }
  ...
  return new Response(null, {
    status: 303,
    headers: { location: appendNotice(returnTo, "success", "Resume renewed successfully.") },
  });
};
